...
\author{\IEEEauthorblockN{Marcos Lund}
\IEEEauthorblockA{N° SIU a2408 \\
Buenos Aires, Argentina \\
lundmarcos@gmail.com}
\and
\IEEEauthorblockN{Tomás Agustín Mc Nally}
\IEEEauthorblockA{N° SIU a2420 \\
Buenos Aires, Argentina \\
mcnallytomas@gmail.com}
\and
\IEEEauthorblockN{Gabriela Sol Salazar}
\IEEEauthorblockA{N° SIU a2414 \\
Buenos Aires, Argentina \\
solgab.salazar@gmail.com}
}
...
test (mAP@0.5 entre 0.749 y 0.773), con diferencias de velocidad de
inferencia de hasta un factor 66. Los resultados indican que la calidad de
...
con hardware de bajo costo. Como parte
...
\begin{figure*}[!t]
\centerline{\includegraphics[width=0.88\textwidth]{muestra_dataset.png}}
...
validación para monitorear el entrenamiento y seleccionar el mejor punto de
control, y el de test se reserva para la evaluación final, tanto
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{image_type_distribution.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{bbox_area_distribution.png}}
...
\cite{b2}. Este refinamiento en dos pasos, junto con las propuestas densas
sobre una pirámide de características, le otorga alta calidad de
localización y buen desempeño sobre objetos pequeños, a costa de un mayor
tiempo de inferencia. Se utiliza una implementación basada en un backbone
ResNet-50 con Feature Pyramid Network.
...
procesamiento adecuadas para aplicaciones en tiempo real \cite{b3}. Sus
puntos fuertes son la detección end-to-end, sin post-procesamiento de
supresión de no máximos, y el uso de atención global, que incorpora contexto
de toda la escena, potencialmente útil para fenómenos difusos como el humo.
Se utiliza la variante RT-DETR-L.
...
y matrices de confusión. Todas las métricas reportadas en la
Sección~\ref{sec:resultados} se calculan sobre el conjunto de test, que no
participó del entrenamiento ni de la selección de puntos de control; la
evaluación cualitativa utiliza el mismo conjunto.
...
sobre el conjunto de test, evaluadas en el mejor punto de control de cada
entrenamiento.

\begin{table}[htbp]
\caption{Métricas de detección sobre el conjunto de test}
\begin{center}
...
\hline
YOLO26s      & \textbf{0.786} & 0.700 & 0.740 & \textbf{0.773} & \textbf{0.443} \\
\hline
Faster R-CNN & \textbf{0.786} & \textbf{0.726} & \textbf{0.755} & 0.749 & 0.403 \\
\hline
...
mAP@0.5 entre el mejor y el peor modelo es de apenas 2.4 puntos porcentuales
(0.749 a 0.773) y el de mAP@0.5:0.95 de 4 puntos (0.403 a 0.443). Cinco
...
YOLO26s obtiene los mejores valores de mAP en ambos umbrales (0.773 y 0.443)
y la mejor precisión (0.786, prácticamente empatada con Faster R-CNN),
consistente con ser el YOLO de mayor capacidad evaluado. Faster R-CNN lidera
recall (0.726) y F1 (0.755) pero registra el peor mAP en ambos umbrales
(0.749 y 0.403): su punto de operación clasifica y detecta bien, pero sus
cajas pierden solapamiento frente a las de los YOLO cuando el criterio de
IoU se vuelve más exigente. Dado que cada arquitectura se entrenó
...
ubica entre 0.788 y 0.841 y el de \textit{fire} entre 0.685 y 0.714, una
brecha de más de 10 puntos que ningún modelo logra cerrar. Esta dificultad
...
Las matrices de confusión sobre test refuerzan este diagnóstico: la
confusión entre clases es prácticamente nula (del orden del 1\%), de modo
que el error dominante no es clasificar mal un objeto detectado sino
omitirlo: a confianza $\geq 0.25$ queda sin detectar el 12--18\% de las
cajas de humo y el 14--30\% de las de fuego, según el modelo. Para un
sistema de alerta temprana este es el tipo de error más costoso, por lo que
mejorar el recall constituye la principal línea de mejora.

\subsection{Análisis del entrenamiento}\label{sec:entrenamiento}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{curvas_entrenamiento.png}}
\caption{Evolución por época de mAP@0.5, mAP@0.5:0.95, precisión y recall,
calculados sobre el conjunto de validación durante el entrenamiento de los
cinco modelos.}
\label{fig:curvas}
\end{figure}

La Fig.~\ref{fig:curvas} muestra la evolución por época de las métricas de
validación durante el entrenamiento y revela dinámicas de convergencia muy
distintas entre paradigmas: Faster R-CNN parte desde la
...
dominio, mientras que los YOLO comienzan entre 0.30 y 0.45 y RT-DETR-L por
...
prácticamente indistinguibles en las cuatro métricas, lo que confirma que el
presupuesto de épocas fue suficiente para todos los paradigmas y respalda la
observación anterior: ninguna arquitectura supera de manera sostenida el
techo común. YOLO26s alcanzó su mejor punto de control en la época 40 de 50,
con las métricas ya en meseta.
...
\hline
YOLOv8n      & 3.0  & 50 & 243  & \textbf{457.2} \\
\hline
...
El contraste es marcado: YOLOv8n procesa 457 imágenes por segundo, 66 veces
más que Faster R-CNN (7.0 FPS), y sin embargo lo supera en mAP (0.759 frente
a 0.749 en mAP@0.5 y 0.434 frente a 0.403 en mAP@0.5:0.95). La ventaja de
Faster R-CNN en F1 (+3 puntos sobre YOLOv8n) podría justificar su uso
únicamente en procesamiento diferido; su costo de entrenamiento también es
muy superior (18.5 frente a 4 horas). YOLO26s ofrece un punto intermedio
atractivo: el mejor mAP de la comparación manteniendo 180 FPS. Para el caso
de uso que motiva este trabajo, el monitoreo simultáneo de múltiples cámaras
en tiempo real, los modelos YOLO dominan claramente el compromiso
desempeño-velocidad.
...
modelos sobre imágenes del conjunto de test.
...
nano registran los recalls más bajos (0.688 y 0.690) y Faster R-CNN el más
alto (0.726).

\begin{figure*}[!t]
\centerline{\includegraphics[width=0.6\textwidth]{cualitativa_objetos_pequenos_AoF07964.png}}
...
\begin{figure*}[!t]
\centerline{\includegraphics[width=0.6\textwidth]{cualitativa_solo_fuego_WEB11741.png}}
...
(típicamente entre 0.3 y 0.8). Esta diferencia no afecta el mAP, pero sí a
la práctica: el umbral de alarma de un sistema en producción debe
calibrarse por modelo.
...
magnitud en parámetros convergen a un rango de 2.4 puntos en mAP@0.5 (0.749
a 0.773). El techo común, junto con la evidencia de errores de anotación,
indica que la calidad del dataset condiciona el resultado más que la
capacidad del modelo: el esfuerzo de mejora debería dirigirse a los datos
antes que a arquitecturas más grandes.

\item \textbf{Los modelos livianos de una etapa ofrecen el mejor compromiso
para la aplicación.} YOLOv8n alcanza el 98\% del mejor mAP@0.5:0.95 con 3M
de parámetros y 457 FPS, 66 veces más rápido que Faster R-CNN, al que además
supera en mAP; YOLO26s obtiene el mejor mAP absoluto (0.773 y 0.443)
manteniendo 180 FPS. Para monitoreo de múltiples cámaras en tiempo real los
YOLO son la elección justificada por las métricas; Faster R-CNN solo se
justifica en procesamiento diferido, donde su F1 superior (0.755) y su
ventaja en objetos pequeños compensan su costo.

\item \textbf{El fuego es estructuralmente más difícil de detectar que el
humo.} La brecha de más de 10 puntos de mAP@0.5 entre clases (0.788--0.841
para \textit{smoke} frente a 0.685--0.714 para \textit{fire}) se repite en
...
mientras que las omisiones alcanzan el 12--18\% en humo y el 14--30\% en
fuego según el modelo. Dado que el falso negativo es el error más costoso en
...
temporales puede sesgar los resultados al alza, mientras que los
...
networks,''
in \textit{Advances in Neural Information Processing Systems}, vol. 28,
2015.

\bibitem{b3}
Y. Zhao, W. Lv, S. Xu, J. Wei, G. Wang, Q. Dang, Y. Liu, and J. Chen,
``DETRs beat YOLOs on real-time object detection,''
in \textit{Proc. IEEE/CVF Conf. on Computer Vision and Pattern Recognition
(CVPR)}, pp. 16965--16974, 2024.
